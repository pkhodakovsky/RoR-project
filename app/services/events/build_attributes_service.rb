...
class Events::BuildAttributesService < ApplicationService
  EXLCUDED_DIRECTORIES = ['vendor/', 'node_modules/'].freeze
  ...
  def parent_id
    if @params['framework'] == Event::BROWSER_JS
  ...
  def similar_message?(message)
    StringDifference.percent(message, @params['message']) <= MESSAGE_SIMILARITY_PERCENT
  end
end
